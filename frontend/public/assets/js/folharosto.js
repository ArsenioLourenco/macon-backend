...
//var url_http = ((window.location.hostname != '10.5.2.225') ? 'http://localhost:3000' : 'http://10.5.2.225:5700'),

//Config Server 6
var url_http = ((window.location.hostname != '192.168.40.6') ? 'http://localhost:3000' : 'http://192.168.40.6:5800'),
    limpar_formulario = $('#limpar_formulario'),
...
botao_imprimir.on("click", function() {
    let imputs_folha_activa = $('.fr_folha_a4 .fr_folha.active input'),
        quantidade_imputs_folha_activa = $('.fr_folha_a4 .fr_folha.active input').length,
        inputs_folha_invalid = $('.fr_folha_a4 .fr_folha.active input[fr-data-invalid="invalid-input"]'),
        elementos_prenchidos = 0;

    for (let i = 0; i < (quantidade_imputs_folha_activa); i++) {
    ...
    //console.log(quantidade_imputs_folha_activa, elementos_prenchidos);

    $.each(imputs_folha_activa, function(index, value) {
        let _element = $(this),
            _element_value = _element.val(),
            _element_value_number = _element.val().length,

            _element_min_value = (typeof _element.attr('minlength') != 'undefined') ? _element.attr('minlength') : null,
            _element_max_value = (typeof _element.attr('maxlength') != 'undefined') ? _element.attr('maxlength') : null;

        if (_element_max_value != null && _element_max_value < _element_value_number || _element_min_value != null && _element_min_value > _element_value_number) {
            _element.attr('fr-data-invalid', 'invalid-input');
        } else {
            if (_element_value == null || _element_value == "" || _element_value == " " || typeof _element_value == 'undefined') {
                _element.attr('fr-data-invalid', 'invalid-input');
            } else {
                _element.removeAttr('fr-data-invalid');
            }
        }


        //console.log(_element, _element_value, _element_value_number, _element_min_value, _element_max_value);
    });

    if (quantidade_imputs_folha_activa == elementos_prenchidos && inputs_folha_invalid.length == 0) {
        window.print();
    } else {
...
function adicionar_usuario(usuario, email, senha) {
    ...
    let url = url_http + '/auth/register/';
...
function autenticacao(usuario, senha) {
    ...
    alert('Efetuando Login de:',  usuario)

    let url = url_http + '/auth/session';
...
function selecionar_usuario(id) {
    ...
    let url = url_http + '/auth/list/user/' + id;
...
function eliminar_usuario(id) {
    ...
    let url = url_http + '/auth/delete/user/' + id;
...
function alternar_acesso(id) {
    ...
    let url = url_http + '/auth/edit/access';
...
$('.fr_imput_cartao').mask('0000000000000000');
...
$(document).on('click', function() {
    let _fr_folha = $(document).find('.fr_folha.active');

    _fr_folha.on('keypress keyup', '.fr_a4_input', function() {
        $(this).removeAttr('fr-data-invalid');
    });

    _fr_folha.on('keypress keyup', '.fr_a4_input[data-validation^="date"]', function() {
        _date = $(this).val();

        if (_date.length > 10) {
            _date = ((_date.length > 11) ? _date.substring(2, 12) : _date.substring(1, 11));

            $(this).val(_date);
            //console.log('date to: ' + _date, 'count' + _date.length);
        }
    });

    _fr_folha.on('focusout', '.fr_a4_input[data-validation^="agency"]', function() {
        let _agency = $(this).val(),
            _return = '';

        switch (_agency.length) {
            case 1:
                _return = '00' + _agency;
                break;
            case 2:
...
$('#fr-recharging-page').click(function() {
	confirm_logout = confirm("Tens a certeza que queres sair da Aplicação? Clique em \"OK\" Para confirmar.");
